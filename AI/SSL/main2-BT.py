...
from snoring.dymn_jax.model_jax import get_model, get_projection_head, BTHead
...
class BarlowTwinsLoss:
    ...
    def __call__(self, z1, z2):
        ...
        z1_norm = (z1 - jnp.mean(z1, axis=0)) / jnp.std(z1, axis=0)
        z2_norm = (z2 - jnp.mean(z2, axis=0)) / jnp.std(z2, axis=0)

        cross_corr = jnp.matmul(z1_norm.T, z2_norm) / self.batch_size

        on_diag = jnp.diagonal(cross_corr).add(-1).pow(2).sum()
...
class FlaxLightning(pl.LightningModule):
    ...
    def on_fit_start(self) -> None:
        ...
        # -----------------------------------------
        params_key, dropout_key = jax.random.split(params_key)
        key = {'params': params_key}
